...
ros::Publisher pub_ELSED;// llg add for 2D-3D in prior point map
...
//    cv::waitKey(1);
    ...
    frame_cnt++;
    trackerData.readImage(show_img.rowRange(0 , ROW));   // rowRange(i,j) 取图像的i～j行 ///llg modify
...
int main(int argc, char **argv)
{
    ...
    pub_ELSED = n.advertise<sensor_msgs::PointCloud>("ELSED", 1000);// llg add for 2D-3D
    /*
    if (SHOW_TRACK)
